...
public class service_appointment_bill extends HFrame{
    ...
    service_appointment_bill()
    {
    	this("SBID_1");
    }
    service_appointment_bill(String ID)
	{
		currenaid = ID;
		setSize(900,700);
        ...
        l2 = new JLabel("Outpatient Service Bill--------------------------------------------------------"
        		+ "------------------------------------------------------------");
        ...
        action[4].addActionListener(new ActionListener() { //search
            ...
            public void actionPerformed(ActionEvent e) {
            	...
            	service_appointment_bill_all rall = new service_appointment_bill_all();
	...
	public String newid()
	{
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				ResultSet rs = stmt.executeQuery("SELECT * from service_appointment_bill where appointment_bill_id = (SELECT max(appointment_bill_id) from service_appointment_bill)");
				if(rs.next()){
					String id = rs.getString("appointment_bill_id");
					String[] sourceArray = id.split("_");
					int myid = Integer.parseInt(sourceArray[1])+1;
					String newid = "SBID_"+myid;
					return newid;
				}
				else{
					return "SBID_1";
	...
	public String minID(){
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				ResultSet rs = stmt.executeQuery("SELECT * from service_appointment_bill where appointment_bill_id = (SELECT min(appointment_bill_id) from service_appointment_bill)");
				while(rs.next()){
					String id = rs.getString("appointment_bill_id");
					return id;
				}
	...
	public void getInfo(String thibid)
	{
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				ResultSet rs = stmt.executeQuery("SELECT * from service_appointment_bill where appointment_bill_id like '"+thibid+"'");
				while(rs.next()){
					bid.setText(thibid);
	...
	public void addnew()
	{
		...
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				int rs = stmt.executeUpdate("insert into service_appointment_bill (appointment_bill_id,patient_id,appointment_id,bill_date,appointment_charge,hospital_charge,grand_total,discount,net_value,status) "
	...
	public void save()
	{
		...
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				int rs = stmt.executeUpdate("update service_appointment_bill set "
	...
	public void get_charge()
	{
		...
		try {
			...
			try{
				Statement stmt2 = (Statement) con.createStatement();
				ResultSet rs2 = stmt2.executeQuery("select * from service_appointment where appointment_id like '"+appointment+"'");
				while(rs2.next()){
					service = rs2.getString("hospital_service_id");
				}
				Statement stmt1 = (Statement) con.createStatement();
				ResultSet rs1 = stmt1.executeQuery("select * from services where service_id like '"+service+"'");
				while(rs1.next()){
					charge = rs1.getInt("charge_for_service");						
				}
				con.commit();
			}catch(Exception e){
				con.rollback();
