...
public class appointment_bill_all extends HFrame{
	//JPanel panel;
	JLabel title,sfor,stext;
	JButton search,refresh,close;
	JTextField searchText;
	JComboBox searchFor;
	appointment_bill_all()
	{
		setSize(900,700);
        ...
        close.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
            	dispose();
            	appointment_bill om = new appointment_bill("DBID_1");
        		om.setVisible(true);
	...
	public void showall()
	{
		...
		String selectsql;
		if(seartext.equals(""))
			selectsql = "select * from appointment_bill";
		else{
			String type="";
			...
			if(t == "net value")
				type = "net_value";
			selectsql = "select * from appointment_bill where "+type+" like '"+seartext+"'";
		}
		...
		try {
			...
			try{
				...
				ResultSet rs1 = stmt1.executeQuery("select count(*) from appointment_bill");
				if(rs1.next()){
					int n = rs1.getInt(1);
