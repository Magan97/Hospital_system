...
public class service_appointment_bill_payments extends HFrame{
	//JPanel panel;
    ...
    service_appointment_bill_payments()
    {
    	this("SPID_1");
    }
    service_appointment_bill_payments(String ID)
	{
		currenaid = ID;
		setSize(900,720);
        ...
        title = new JLabel("Outpatient Service Bill Payments");
        title.setBounds(200, 50, 400, 50);
        title.setFont(new Font("Dialog", 1, 25));
        title.setSize(new Dimension(450,50));
        panel.add(title);
        
        divide1 = new JLabel("Outpatient Service Bill Payments--------------------------------------------------------"
        		+ "------------------------------------------------------------");
        ...
        action[4].addActionListener(new ActionListener() { //arch
            ...
            public void actionPerformed(ActionEvent e) {
            	dispose();
            	//
            	ViewAllOutService ViewAllOutService1 = new ViewAllOutService();
    ...
    public void geaid()
	{
		try {
			...
			try{
				...
				ResultSet rs = stmt.executeQuery("select * from service_appointment_bill");
				while(rs.next()){
    ...
    public void getbillid()
	{
		...
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				ResultSet rs = stmt.executeQuery("select * from service_appointment_bill where patient_id = '"+patid+"'");
				while(rs.next()){
    ...
    public String newid()
	{
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				ResultSet rs = stmt.executeQuery("SELECT * from service_appointment_bill_payment where bill_payment_id = (SELECT max(bill_payment_id) from service_appointment_bill_payment)");
				if(rs.next()){
	...
	public void getInfo(String thibid)
	{
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				ResultSet rs = stmt.executeQuery("SELECT * from service_appointment_bill_payment where bill_payment_id like '"+thibid+"'");
				while(rs.next()){
					bid.setText(thibid);
					appo_id.addItem(rs.getString("appointment_bill_id"));
					appo_id.setSelectedItem(rs.getString("appointment_bill_id"));
	...
	public void addnew()
	{
		...
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				int rs = stmt.executeUpdate("insert into service_appointment_bill_payment (bill_payment_id,appointment_bill_id,amount_paid,paid_date,payment_type,cheque_no,cheque_date,bank) "
	...
	public void save()
	{
		...
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				int rs = stmt.executeUpdate("update service_appointment_bill_payment set "
						+ "appointment_bill_id='"+info[1]+"',amount_paid='"+info[2]+"',paid_date='"+info[3]
	...
	public void get_charge()
	{
		...
		try {
			...
			try{
				Statement stmt2 = (Statement) con.createStatement();
				ResultSet rs2 = stmt2.executeQuery("select * from service_appointment_bill where appointment_bill_id like '"+bill+"'");
				while(rs2.next()){
					...
					status.setSelectedItem(rs2.getString("status"));
	...
	{
		String selectsql;
		selectsql = "select * from service_appointment_bill_payment";
		String[] columnName = {"NO","AMOUNT PAID","PAID DATE","PAY TYPE","CEDIT CARD/CHEQUE","CHEQUE DATE","BANK","Status"};
		Object[][] data1 = null;
		try {
			//Class.forName(com.mysql.jdbc.Driver.class.getName());
			String url = "jdbc:mysql://localhost/hospital_system";
			...
			try{
				Statement stmt1 = (Statement) con.createStatement();
				ResultSet rs1 = stmt1.executeQuery("select count(*) from service_appointment_bill_payment");
				if(rs1.next()){
					int n = rs1.getInt(1);
				...
				while(rs2.next()){
					...
					data1[i][1] = rs2.getString("appointment_bill_id");
