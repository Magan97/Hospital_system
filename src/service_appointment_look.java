...
public class service_appointment_look extends HFrame{
	...
	service_appointment_look()
    {
    	this("APID_1");
    }
	service_appointment_look(String ID)
	{
		currentID = ID;
		setSize(900,700);
        ...
        title = new JLabel("Service Appointment");
        title.setBounds(300, 50, 400, 50);
        title.setFont(new Font("Dialog", 1, 25));
        title.setSize(new Dimension(450,50));
        panel.add(title);
        
        l2 = new JLabel("Hospital Service Details--------------------------------------------------------");
        ...
        action[0].addActionListener(new ActionListener() { //add
            ...
            public void actionPerformed(ActionEvent e) {
            	dispose();
            	Service_appointment a = new Service_appointment();
            	a.setVisible(true);
        ...
        action[4].addActionListener(new ActionListener() { //search
            ...
            public void actionPerformed(ActionEvent e) {
            	dispose();
            	service_appointment_look_all rall = new service_appointment_look_all();
	...
	{
		String[] columnName = {"service id","service name","duration","charge","note","status"};
		Object[][] data1 = null;
		try {
			//Class.forName(com.mysql.jdbc.Driver.class.getName());
			String url = "jdbc:mysql://localhost/hospital_system";
	...
	public int correct_date()
	{
		...
		try {
			...
			try{
				Statement stmt2 = (Statement) con.createStatement();
				ResultSet rs2 = stmt2.executeQuery("select * from service_schedule_details where service_schedule_id like '"+docid+"'");
				while(rs2.next()){
	...
	public String minID(){
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				ResultSet rs = stmt.executeQuery("SELECT * from service_appointment where appointment_id = (SELECT min(appointment_id) from service_appointment)");
				while(rs.next()){
					String id = rs.getString("appointment_id");
					return id;
				}
	...
	public void getInfo(String thihid)
	{
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				ResultSet rs = stmt.executeQuery("SELECT * from service_appointment where appointment_id like '"+thihid+"'");
				while(rs.next()){
					a_id.setText(thihid);
					pat_id.setSelectedItem(rs.getString("patient_id"));
					doc_id.setSelectedItem(rs.getString("hospital_service_id"));
	...
	public void save()
	{
		...
		if(t.after(text) == true)
		{
			...
			try {
				...
				try{
					Statement stmt = (Statement) con.createStatement();
					int rs = stmt.executeUpdate("update service_appointment set "
							+ "patient_id='"+info[1]+"',hospital_service_id='"+info[2]+"',appointment_date='"+info[3]+"',appointment_time='"+info[4]+"',status='"+info[5]+"'where appointment_id like '"+info[0]+"'");
					if(rs > 0)
