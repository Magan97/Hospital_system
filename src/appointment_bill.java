...
public class appointment_bill extends HFrame{  // for doctor appointment
	...
	String currenaid;
	String[] labels1 = {"Appointment bill id:","patient id:","appointment id:","bill date:","appointment charge:"};
	String[] labels2 = {"hospital charge:","grand total:","discount:","net value:","status:"};
	JTextField bid,app_c,hos_c,text,grand,discount,net;
    ...
    appointment_bill(String ID)
	{
        ...
        title = new JLabel("Outpatient Doctor Bill");
        title.setBounds(300, 50, 400, 50);
        title.setFont(new Font("Dialog", 1, 25));
        title.setSize(new Dimension(450,50));
        panel.add(title);
        
        l2 = new JLabel("Outpatient Doctor Bill--------------------------------------------------------"
        		+ "------------------------------------------------------------");
        ...
        appo_id.setBounds(220, 260, 120, 20);
        panel.add(appo_id);
        ...
        text.getDocument().addDocumentListener(new DocumentListener() {
        	 public void insertUpdate(DocumentEvent e) {
                 ...
                 try {
                     Date dt1 = df.parse(text.getText());
                     Date dt2 = new Date();
                     if (dt1.getTime() > dt2.getTime()) {
                         System.out.println("error");
                         text.setText("2017-12-19");
                     } else  {
                         System.out.println("dt1ÔÚdt2ºó");
                     } 
        ...
        */
        ...
        app_c = new JTextField("");
        app_c.setBounds(220, 380, 150, 20);
        panel.add(app_c);
        ...
        action[4].addActionListener(new ActionListener() { //search
            ...
            public void actionPerformed(ActionEvent e) {
            	dispose();           	
            	appointment_bill_all rall = new appointment_bill_all();
	...
	public String newid()
	{
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				ResultSet rs = stmt.executeQuery("SELECT * from appointment_bill where appointment_bill_id = (SELECT max(appointment_bill_id) from appointment_bill)");
				if(rs.next()){
					String id = rs.getString("appointment_bill_id");
					String[] sourceArray = id.split("_");
					int myid = Integer.parseInt(sourceArray[1])+1;
					String newid = "DBID_"+myid;
					return newid;
				}
				else{
					return "DBID_1";
	...
	public String minID(){
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				ResultSet rs = stmt.executeQuery("SELECT * from appointment_bill where appointment_bill_id = (SELECT min(appointment_bill_id) from appointment_bill)");
				while(rs.next()){
					String id = rs.getString("appointment_bill_id");
					return id;
				}
	...
	public void getInfo(String thibid)
	{
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				ResultSet rs = stmt.executeQuery("SELECT * from appointment_bill where appointment_bill_id like '"+thibid+"'");
				while(rs.next()){
					bid.setText(thibid);
					inpat_id.setSelectedItem(rs.getString("patient_id"));
					appo_id.addItem(rs.getString("appointment_id"));
					appo_id.setSelectedItem(rs.getString("appointment_id"));
					text.setText(rs.getString("bill_date"));
					app_c.setText(rs.getString("appointment_charge"));
	...
	public void addnew()
	{
		...
		info[2] = appo_id.getSelectedItem().toString();
		info[3] = text.getText();
		info[4] = app_c.getText();
		...
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				int rs = stmt.executeUpdate("insert into appointment_bill (appointment_bill_id,patient_id,appointment_id,bill_date,appointment_charge,hospital_charge,grand_total,discount,net_value,status) "
	...
	public void save()
	{
		...
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				int rs = stmt.executeUpdate("update appointment_bill set "
						+ "patient_id='"+info[1]+"',appointment_id='"+info[2]+"',bill_date='"+info[3]
								+"',appointment_charge='"+info[4]+"',hospital_charge='"+info[5]+"',grand_total='"+info[6]+"',discount='"+info[7]+"',net_value='"+info[8]+"',status='"+info[9]+"'where appointment_bill_id like '"+info[0]+"'");
				if(rs > 0)
	...
	public void trtable()
	{
        ...
        panel1 = new JPanel();
        panel1.setLayout(null);
        JLabel title1 = new JLabel("Appointment");
        title1.setBounds(140, 20, 150, 80);
        l.add(title1);
        String patient = inpat_id.getSelectedItem().toString();
		String[] columnName = {"appointment id","patient id","doctor id","Date/Time","status"};
		Object[][] data1 = null;
		try {
			//Class.forName(com.mysql.jdbc.Driver.class.getName());
			String url = "jdbc:mysql://localhost/hospital_system";
			...
			try{
				Statement stmt1 = (Statement) con.createStatement();
				ResultSet rs1 = stmt1.executeQuery("select count(*) from doctor_appointment where patient_id like '"+patient+"'");
				if(rs1.next()){
					int n = rs1.getInt(1);
					num+=n;
				}
				data1 = new Object[num][5];
				Statement stmt2 = (Statement) con.createStatement();
				ResultSet rs2 = stmt2.executeQuery("select * from doctor_appointment where patient_id like '"+patient+"'");
				while(rs2.next()){
	...
	public void get_appointment()
	{
		...
		try {
			...
			try{
				...
				ResultSet rs2 = stmt2.executeQuery("select * from doctor_appointment where patient_id like '"+patient+"'");
				while(rs2.next()){
					String id = rs2.getString("appointment_id");
					aid.add(id);
				}
				con.commit();
			}catch(Exception e){
	...
	{
		String appointment = appo_id.getSelectedItem().toString();
		String doctor="";
		int charge=0;
		int hosc=20;
		int disc=9;
		try {
			...
			try{
				Statement stmt2 = (Statement) con.createStatement();
				ResultSet rs2 = stmt2.executeQuery("select * from doctor_appointment where appointment_id like '"+appointment+"'");
				while(rs2.next()){
					doctor = rs2.getString("doctor_id");
				}
				Statement stmt1 = (Statement) con.createStatement();
				ResultSet rs1 = stmt1.executeQuery("select * from doctor_details where doctor_id like '"+doctor+"'");
				while(rs1.next()){
					charge = rs1.getInt("doctor_vcharge");						
				}
				con.commit();
			}catch(Exception e){
				con.rollback();
		...
		app_c.setText(charge+"");
		hos_c.setText("20");
		grand.setText(charge+hosc+"");
