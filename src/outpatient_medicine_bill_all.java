...
public class outpatient_medicine_bill_all extends HFrame{
	//JPanel panel;
	JLabel title,sfor,stext;
	JButton search,refresh,close;
	JTextField searchText;
	JComboBox searchFor;
	outpatient_medicine_bill_all()
	{
		setSize(900,700);
        ...
        close.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
            	dispose();
            	outpatient_medicine_bill om = new outpatient_medicine_bill("MBID_1");
        		om.setVisible(true);
        ...
        String[] search1 = {"medicine bill id","patient id","treatment id","bill date","medicine charge","hospital charge","grand total","discount","net value","status"};
	...
	public void showall()
	{
		...
		String selectsql;
		if(seartext.equals(""))
			selectsql = "select * from outpatient_medicine_bill";
		else{
			String type="";
			if(t == "medicine bill id")
				type = "medicine_bill_id";
			if(t == "patient id")
				type = "patient_id";
			if(t == "treatment id")
				type = "treatment_id";
			if(t == "bill date")
				type = "bill_date";
			if(t == "medicine charge")
				type = "medicine_charge";
			...
			if(t == "net value")
				type = "net_value";
			selectsql = "select * from outpatient_medicine_bill where "+type+" like '"+seartext+"'";
		}
		System.out.println(selectsql);
		String[] columnName = {"medicine bill id","patient id","treatment id","bill date","medicine charge","hospital charge","grand total","discount","net value","status"};
		Object[][] data1 = null;
		try {
			...
			try{
				...
				ResultSet rs1 = stmt1.executeQuery("select count(*) from outpatient_medicine_bill");
				if(rs1.next()){
					int n = rs1.getInt(1);
				...
				while(rs2.next()){
					data1[i][0] = rs2.getString("medicine_bill_id");
					data1[i][1] = rs2.getString("patient_id");
					data1[i][2] = rs2.getString("treatment_id");
					data1[i][3] = rs2.getString("bill_date");
					data1[i][4] = rs2.getString("medicine_charge");
