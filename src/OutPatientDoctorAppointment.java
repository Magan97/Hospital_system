...
public class OutPatientDoctorAppointment extends HFrame{
	//JPanel panel;
    ...
    OutPatientDoctorAppointment(String ID)
	{
		currenaid = ID;
		setSize(900,720);
        ...
        title = new JLabel("Outpatient Doctor Bill Payments");
        title.setBounds(200, 50, 400, 50);
        title.setFont(new Font("Dialog", 1, 25));
        title.setSize(new Dimension(450,50));
        panel.add(title);
        
        divide1 = new JLabel("Outpatient Doctor Bill Payments--------------------------------------------------------"
        		+ "------------------------------------------------------------");
        ...
        action[4].addActionListener(new ActionListener() { //arch
            ...
            public void actionPerformed(ActionEvent e) {
            	dispose();
            	//
            	ViewAllOutDoctor ViewAllOutDoctor = new ViewAllOutDoctor();
            	ViewAllOutDoctor.setVisible(true);
            	ViewAllOutDoctor.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); 
            }
        });
    ...
    public void geaid()
	{
		try {
			...
			try{
				...
				ResultSet rs = stmt.executeQuery("select * from appointment_bill");
				while(rs.next()){
    ...
    public void getbillid()
	{
		...
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				ResultSet rs = stmt.executeQuery("select * from appointment_bill where patient_id = '"+patid+"'");
				while(rs.next()){
    ...
    public String newid()
	{
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				ResultSet rs = stmt.executeQuery("SELECT * from appointment_bill_payment where bill_payment_id = (SELECT max(bill_payment_id) from appointment_bill_payment)");
				if(rs.next()){
					String id = rs.getString("bill_payment_id");
					String[] sourceArray = id.split("_");
					int myid = Integer.parseInt(sourceArray[1])+1;
					String newid = "DPID_"+myid;
					return newid;
				}
				else{
					return "DPID_1";
	...
	public String minID(){
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				ResultSet rs = stmt.executeQuery("SELECT * from appointment_bill_payment where bill_payment_id = (SELECT min(bill_payment_id) from appointment_bill_payment)");
				while(rs.next()){
					String id = rs.getString("bill_payment_id");
					return id;
				}
	...
	public void getInfo(String thibid)
	{
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				ResultSet rs = stmt.executeQuery("SELECT * from appointment_bill_payment where bill_payment_id like '"+thibid+"'");
				while(rs.next()){
					bid.setText(thibid);
	...
	public void addnew()
	{
		...
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				int rs = stmt.executeUpdate("insert into appointment_bill_payment (bill_payment_id,appointment_bill_id,amount_paid,paid_date,payment_type,cheque_no,cheque_date,bank) "
	...
	public void save()
	{
		...
		try {
			...
			try{
				Statement stmt = (Statement) con.createStatement();
				int rs = stmt.executeUpdate("update appointment_bill_payment set "
	...
	public void get_treatment()
	{
		...
		try {
			...
			try{
				Statement stmt2 = (Statement) con.createStatement();
				ResultSet rs2 = stmt2.executeQuery("select * from appointment_bill where patient_id like '"+patient+"'");
				System.out.print(patient+"---------------");
				while(rs2.next()){
					
					String id = rs2.getString("appointment_bill_id");
					aid.add(id);
				}
				con.commit();
			}catch(Exception e){
	...
	public void get_charge()
	{
		...
		try {
			...
			try{
				Statement stmt2 = (Statement) con.createStatement();
				ResultSet rs2 = stmt2.executeQuery("select * from appointment_bill where appointment_bill_id like '"+bill+"'");
				while(rs2.next()){
	...
	{
		String selectsql;
		selectsql = "select * from appointment_bill_payment";
		...
		try {
			...
			try{
				Statement stmt1 = (Statement) con.createStatement();
				ResultSet rs1 = stmt1.executeQuery("select count(*) from appointment_bill_payment");
				if(rs1.next()){
					int n = rs1.getInt(1);
