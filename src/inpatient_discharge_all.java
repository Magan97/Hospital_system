...
public class inpatient_discharge_all extends HFrame{
	//JPanel panel;
	JLabel title,sfor,stext;
	JButton search,refresh,close;
	JTextField searchText;
	JComboBox searchFor;
	inpatient_discharge_all()
	{
		setSize(900,700);
        ...
        title = new JLabel("Inpatient Discharge All");
        ...
        close.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
            	dispose();
            	discharge_details rd = new discharge_details("DCID_1");
        		rd.setVisible(true);
        ...
        String[] search1 = {"discharge id","admission id","discharge date","discharge time","Status"};
	...
	public void showall()
	{
		...
		String selectsql;
		if(seartext.equals(""))
			selectsql = "select * from inpatient_discharge";
		else{
			String type="";
			if(t == "discharge id")
				type = "discharge_id";
			...
			if(t == "Status")
				type = "status";
			selectsql = "select * from inpatient_discharge where "+type+" like '"+seartext+"'";
		}
		System.out.println(selectsql);
		String[] columnName = {"discharge id","admission id","discharge date","discharge time","Status"};
		Object[][] data1 = null;
		try {
			...
			try{
				...
				ResultSet rs1 = stmt1.executeQuery("select count(*) from inpatient_discharge");
				if(rs1.next()){
					int n = rs1.getInt(1);
				...
				while(rs2.next()){
					data1[i][0] = rs2.getString("discharge_id");
					data1[i][1] = rs2.getString("admission_id");
					data1[i][2] = rs2.getString("discharge_date");
					data1[i][3] = rs2.getString("discharge_time");
