...
public class doctor_appointment_look_all extends HFrame{
	//JPanel panel;
	JLabel title,sfor,stext;
	JButton search,refresh,close;
	JTextField searchText;
	JComboBox searchFor;
	doctor_appointment_look_all()
	{
		setSize(900,700);
        ...
        String[] search1 = {"appointment id","patient id","doctor id","appointment date","appointment time","Status"};
	...
	public void showall()
	{
		...
		String selectsql;
		if(seartext.equals(""))
			selectsql = "select * from doctor_appointment";
		else{
			String type="";
			if(t == "appointment id")
				type = "appointment_id";
			...
			selectsql = "select * from doctor_appointment where "+type+" like '"+seartext+"'";
		}
		System.out.println(selectsql);
		String[] columnName = {"appointment id","patient id","doctor id","appointment date","appointment time","Status"};
		Object[][] data1 = null;
		try {
			...
			try{
				...
				ResultSet rs1 = stmt1.executeQuery("select count(*) from doctor_appointment");
				if(rs1.next()){
					int n = rs1.getInt(1);
